...
3. For each set in `[a,b,c]`, build a full-design prompt (vary composition / time-of-day / focal element across sets). Pass the Hebrew headline as **raw text, no quotes**:
   ...
   **Banner full-design prompt (target 310×600 → aspect 9:16):**
   ...
   Near the bottom, add EXACTLY ONE rounded CALL-TO-ACTION button (only one — do not repeat it) with a GLOSSY/METALLIC finish in the accent color — a subtle bright specular highlight/sheen streak across the top, soft reflective shine, premium polished look (not flat/matte) — contrasting text, short Hebrew CTA, right-to-left, centered: {cta}   # e.g. הזמינו אונליין / הזמינו כעת — a booking CTA is MANDATORY on every banner (§9.6 Mode B). The glossy/shine treatment is the client's preferred CTA style (confirmed live on המושבה, 2026-07-02) — use it by default on every banner CTA, not just here.
